...
import { getPriceSuggestionById, updatePriceSuggestionById } from 'apiSdk/price-suggestions';
...
function PriceSuggestionEditPage() {
  ...
  const handleSubmit = async (values: PriceSuggestionInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updatePriceSuggestionById(id, values);
      mutate(updated);
      resetForm();
