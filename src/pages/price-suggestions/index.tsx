...
import { getPriceSuggestions, deletePriceSuggestionById } from 'apiSdk/price-suggestions';
...
function PriceSuggestionListPage() {
  const { hasAccess } = useAuthorizationApi();
  const { data, error, isLoading, mutate } = useSWR<PriceSuggestionInterface[]>(
    () => '/price-suggestions',
    () =>
      getPriceSuggestions({
  ...
  const handleView = (id: string) => {
    if (hasAccess('price_suggestion', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/price-suggestions/view/${id}`);
    }
  };
  ...
  return (
    <AppLayout>
      <Box bg="white" p={4} rounded="md" shadow="md">
        {hasAccess('price_suggestion', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <Flex justifyContent="space-between" mb={4}>
            <Text as="h1" fontSize="2xl" fontWeight="bold">
              Price Suggestion
            </Text>
            <NextLink href={`/price-suggestions/create`} passHref legacyBehavior>
                  ...
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    <Td>
                      {hasAccess('price_suggestion', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                        <NextLink href={`/price-suggestions/edit/${record.id}`} passHref legacyBehavior>
                          <Button
