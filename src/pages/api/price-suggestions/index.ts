...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    case 'GET':
      return getPriceSuggestions();
    case 'POST':
      return createPriceSuggestion();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getPriceSuggestions() {
    const data = await prisma.price_suggestion
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'price_suggestion'));
  ...
  async function createPriceSuggestion() {
    ...
    const data = await prisma.price_suggestion.create({
      data: body,
    });
    return res.status(200).json(data);
  }
